...
# -----------------------------
D_real = 0.08
f_pixels = 570  # Hardcoded effective focal length

lower_blue = np.array([95, 120, 70])
upper_blue = np.array([130, 255, 255])

# --- DISTANCE LIMIT ---
# The robot will IGNORE anything farther than this (in meters)
MAX_DISTANCE = 2.5  
...
HOUGH_PARAM1 = 100
HOUGH_PARAM2 = 30 
MIN_RADIUS = 15  # Kept this to filter tiny noise
MAX_RADIUS = 200
...
# -----------------------------
bridge = CvBridge()
# fx, fy, intrinsics_received removed since we use f_pixels
...
def image_callback(msg):
    ...
    # Get dimensions dynamically from the frame
    ...
    if best_circle is not None:
        ...
        # Calculate Distance using f_pixels
        Z_raw = (D_real * f_pixels) / D_pixel
    ...
    if valid_detection:
        ...
        # Calculate X, Y using f_pixels and dynamic center (cx, cy)
        X_raw = (u - cx) * Z_raw / f_pixels
        Y_raw = (v - cy) * Z_raw / f_pixels
